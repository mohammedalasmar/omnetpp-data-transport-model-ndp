...
//        sendBytes = &par("sendBytes");  // volatile  moh new
        ...
        std::cout << "TCPSessionApp initialize -- tOpen =   " << tSend << "\n";
        std::cout << "TCPSessionApp initialize -- tSend =   " << tSend << "\n";
        std::cout << "TCPSessionApp initialize -- sendBytes  " << sendBytes << "\n";
        const char *connectAddress = par("connectAddress");
        std::cout << "TCPSessionApp initialize -- connectAddress  " << connectAddress << "\n";
        int connectPort = par("connectPort");
        std::cout << "TCPSessionApp initialize -- connectPort " << connectPort << "\n";
        ...
        // moh : commented see TCPSessionApp::handleParameterChange
        if (sendBytes > 0)
            commands.push_back(Command((simtime_t) tSend, sendBytes));
        if (commands.size() == 0)
...
//            mohMsg->setKind(MSGKIND_CONNECT);  // add moh
            scheduleAt((simtime_t) tOpen, timeoutMsg);  // moh comment this line as we need to start the connection based on handleParameterChange()
        }
    }
}
...
bool TCPSessionApp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    ...
    if (dynamic_cast<NodeStartOperation *>(operation)) {
        if ((NodeStartOperation::Stage) stage == NodeStartOperation::STAGE_APPLICATION_LAYER) {
            if (simTime() <= (simtime_t) tOpen) {
                timeoutMsg->setKind(MSGKIND_CONNECT);
                scheduleAt((simtime_t) tOpen, timeoutMsg);
            }
        }
    }
...
{
//    std::cout<<"TCPSessionApp::handleTimer " << msg->getKind() << "\n" ;
...
//    std::cout <<"sendData() \n";
    ...
    if (++commandIndex < (unsigned int) commands.size()) {
...
// std::cout << " tclose  === " <<  simTime() << std::endl;
    }
}
