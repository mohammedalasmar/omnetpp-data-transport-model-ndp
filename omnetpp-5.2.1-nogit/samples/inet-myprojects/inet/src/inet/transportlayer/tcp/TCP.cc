...
//simsignal_t numRTOsMOH = registerSignal("numRTOsMOH");
...
// MOH ADDED
simsignal_t numRTOsMOH = tcp::TCP::registerSignal("numRTOsMOH");
simsignal_t appGateIndexMOH = tcp::TCP::registerSignal("appGateIndexMOH");
simsignal_t numFastRetransmissions = tcp::TCP::registerSignal("numFastRetransmissions");
...
//            std::cout << "\n\n\n\n current node: "<< cSimulation::getActiveSimulation()->getContextSimpleModule()->getFullPath() << std::endl;
//            for (auto iter = tcpAppConnMap.begin(); iter != tcpAppConnMap.end(); ++iter) {
//                 std::cout << "  tcpAppConnMap  sec" << iter->second  << " ==> ";
//                 std::cout << "  tcpAppConnMap  first " << iter->first << "\n";
//             }
...
void TCP::finish()
{
    ...
    // MOH numRTOs
//    std::cout << "\n\n\n\n TCP FINISH: current node: " << cSimulation::getActiveSimulation()->getContextSimpleModule()->getFullPath() << std::endl;
//    std::cout << "mappppp:                 \n";
    for (std::map<int, int>::iterator iter =  appGateIndexTimeOutMap.begin(); iter !=  appGateIndexTimeOutMap.end(); ++iter) {
//        std::cout << "FLOW INDEX(appGateIndex): " << iter->first << " ===> ";
//        std::cout << "  numRtos:      " << iter->second << " \n";
        emit(numRTOsMOH, iter->second);
    }

    // MOH num Fast Retransmissions
    for (std::map<int, int>::iterator iter =  numFastRetransmissionsMap.begin(); iter !=  numFastRetransmissionsMap.end(); ++iter) {
//        std::cout << "FLOW INDEX(appGateIndex): " << iter->first << " ===> ";
//        std::cout << "  numFastRetransmissions:      " << iter->second << " \n";
        emit(numFastRetransmissions, iter->second);
    }


    // MOH signals ???????
//    emit(appGateIndexMOH, conn->appGateIndex);
