...
#ifndef __INET_NDPBASEALG_H
#define __INET_NDPBASEALG_H
...
/**
 * Includes basic NDP algorithms: adaptive retransmission, PERSIST timer,
 * keep-alive, delayed acks -- EXCLUDING congestion control. Congestion
 * control is implemented in subclasses such as NDPTahoeAlg or NDPRenoAlg.
 *
 * Implements:
 ...
 * NDPAlgorithm which uses ticks (or rather, factor out timer handling to
 ...
 */
class INET_API NDPBaseAlg : public NDPAlgorithm
{
  protected:
    NDPBaseAlgStateVariables *& state;    // alias to NDPAlgorithm's 'state'
 ...
 //    int numSymbolsToGet;
  ...
  protected:
    ...
    //@{
    virtual void processRexmitTimer(NDPEventCode& event);
    virtual void processPersistTimer(NDPEventCode& event);
    virtual void processDelayedAckTimer(NDPEventCode& event);
    virtual void processKeepAliveTimer(NDPEventCode& event);
    //@}
    ...
    /** Utility function */
    cMessage *cancelEvent(cMessage *msg) { return conn->getNDPMain()->cancelEvent(msg); }
...
#endif // ifndef __INET_NDPBASEALG_H
