...
//            MY_COUT <<  "  requestsGap=  " << requestsGap << std::endl;
        state->isLongFlow = ndpseg->getIsLongFlow() ;
//       if( ndpseg->getIsLongFlow() == true ){ // ndpseg->getIsLongFlow() == true
//
//            char msgname[16];
//            int sequenceNo = 1; // state->se +1 ;
//            sprintf(msgname, "PKT-%d", sequenceNo);
//            GenericAppMsgNdp *datamsg = new GenericAppMsgNdp(msgname);
//            cPacket *msg = PK(datamsg);
//
//            const char *payloadName = msg->getName();
//
//
//            NDPSegment *ndpseg2 = new NDPSegment(nullptr);
//            ndpseg2->addPayloadMessage(msg->dup(), 1);
//            ndpseg2->setName(payloadName);
//            ndpseg2->setDataSequenceNumber(sequenceNo);
//            ndpseg2->setPayloadLength(1500);
//            ndpseg2->setIsDataPacket(true);
//            ndpseg2->setIsPullPacket(false);
//            ndpseg2->setIsHeader(false);
//            ndpseg2->setAckBit(false);
//            ndpseg2->setNackBit(false);
//            ndpseg2->setSynBit(false);
//
//            ndpseg2->setIsLongFlow(state->isLongFlow);
//            ndpseg2->setNumPacketsToSend(state->numPacketsToSend);
//            ndpseg2->setPriorityValue(state->priorityValue);
//
//               sendToIP(ndpseg2);
...
//         MY_COUT  << " \n\n\n\n\n $$$$$$$$$$$$$$$  Receiver $$$$$$$$$$$$$$$$$  HEADER arrived.  " <<  getNDPMain()->getFullPath() << std::endl;
        ...
        if (state->isLongFlow == false) ++state->numRcvTrimmedHeader;
...
//              std::cout << " dddddddd data arrive long flow \n\n\n";

              state->isLongFlow = isLongFlowPacket;
              addRequestToPullsQueue();

              if (numberReceivedPackets == 1) {
                  getNDPMain()->requestCONNMap[getNDPMain()->connIndex] = this;
                     ++getNDPMain()->connIndex;
                     getNDPMain()->sendFirstRequest();
              }
...
//    if (state->isLongFlow)
//        std::cout << "Adding new request to the pull queue -- pullsQueue length now = " << pullQueue.getLength() << "\n\n\n\n";
