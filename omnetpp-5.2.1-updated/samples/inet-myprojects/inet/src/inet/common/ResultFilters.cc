...
//#include "/Volumes/LocalDataHD/m/ma/ma777/Desktop/omnetpp-5.2.1-hpc/omnetpp-5.2.1/samples/inet-myprojects/raptorqimplementation/application/base/ApplicationPacket_m.h"
...
//    std::cout << "wwwwwwwwwwwwwwwwwwwww  \n\n\n\n\n\n\n";
    if (auto packet = dynamic_cast<cPacket *>(object)) {
        ...
        packets++;
//        if (packets >= packetLimit) {// TODO reduce the value of packetLimit, which is =100 now,  if needed

//  MOH: NOTE packetLimit=100 if we just send few packets (less than 100) in short time
//  then the throughput would not be calculated as non of the following conditions will be satisfied
//  in such case the recorded throughput value will be NaN (not a number)
