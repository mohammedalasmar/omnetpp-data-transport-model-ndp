...
void Topology::calculateWeightedSingleShortestPathsTo(Node *_target)
{
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < dest->getNumInLinks(); i++) {
            ...
            // MOH: added- note I added the equal below  ">=" larger or equal
            if (newdist != INFINITY && src->dist >= newdist) {    // it's a valid shorter path from src to target node
                ...
                src->dist = newdist;
                src->outPath = dest->inLinks[i];  // MOH: note not used anymore
                src->outPathEcmp.push_back(dest->inLinks[i]); // MOH: added
                //     for (std::vector<Link *>::iterator j = src->outPathEcmp.begin(); j != src->outPathEcmp.end(); ++j) {
                //          EV_DEBUG << " == == == == outPathEcmp size " <<src->outPathEcmp.size()<< endl;
                //     }
