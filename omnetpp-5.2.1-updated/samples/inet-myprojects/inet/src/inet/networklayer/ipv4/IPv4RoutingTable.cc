    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (isNodeUp) {
            ...
            if (*filename && parser.readRoutingTableFromFile(filename) == -1)
                throw cRuntimeError("Error reading routing table file %s",
                        filename);
        }
...
void IPv4RoutingTable::refreshDisplay() const {
    ...
    if (routerId.isUnspecified())
        sprintf(buf, "%d+%d routes", (int) routes.size(),
                (int) multicastRoutes.size());
    else
        sprintf(buf, "routerId: %s\n%d+%d routes", routerId.str().c_str(),
                (int) routes.size(), (int) multicastRoutes.size());
...
void IPv4RoutingTable::receiveSignal(cComponent *source, simsignal_t signalID,
    ...
    if (signalID == NF_INTERFACE_CREATED) {
        ...
        const InterfaceEntry *entry = check_and_cast<const InterfaceEntry *>(
                obj);
...
void IPv4RoutingTable::deleteInterfaceRoutes(const InterfaceEntry *entry) {
    ...
    for (auto it = multicastRoutes.begin(); it != multicastRoutes.end();) {
        ...
        if (route->getInInterface()
                && route->getInInterface()->getInterface() == entry) {
...
void IPv4RoutingTable::printRoutingTable() const {
    EV << "-- Routing table --\n";
    EV
              << stringf("%-16s %-16s %-16s %-4s %-16s %s\n", "Destination",
                      "Netmask", "Gateway", "Iface", "", "Metric");

    for (int i = 0; i < getNumRoutes(); i++) {
        IPv4Route *route = getRoute(i);
        InterfaceEntry *interfacePtr = route->getInterface();
        EV
                  << stringf("%-16s %-16s %-16s %-4s (%s) %d\n",
                          ...
                          !interfacePtr ?
                                  "*  " :
                                  interfacePtr->ipv4Data()->getIPAddress().str().c_str(),
                          route->getMetric());
    }
    EV << "\n";
}
...
void IPv4RoutingTable::printMulticastRoutingTable() const {
    ...
    EV
              << stringf("%-16s %-16s %-16s %-6s %-6s %s\n", "Source",
                      "Netmask", "Group", "Metric", "In", "Outs");

    for (int i = 0; i < getNumMulticastRoutes(); i++) {
        IPv4MulticastRoute *route = getMulticastRoute(i);
        EV
                  << stringf("%-16s %-16s %-16s %-6d %-6s ",
                          route->getOrigin().isUnspecified() ?
                                  "*" : route->getOrigin().str().c_str(),
                          route->getOriginNetmask().isUnspecified() ?
                                  "*" : route->getOriginNetmask().str().c_str(),
                          route->getMulticastGroup().isUnspecified() ?
                                  "*" :
                                  route->getMulticastGroup().str().c_str(),
                          route->getMetric(),
                          !route->getInInterface() ?
                                  "*" :
                                  route->getInInterface()->getInterface()->getName());
...
InterfaceEntry *IPv4RoutingTable::getInterfaceByAddress(
...
InterfaceEntry *IPv4RoutingTable::findInterfaceByLocalBroadcastAddress(
...
// MOH: comment this method (no need for it anymore) replaced by the next one
// MOH: TODO this method is still called by some Multicasting functions, to be replaced by the findBestMatchingRouteEcmp method
IPv4Route *IPv4RoutingTable::findBestMatchingRoute(
        const IPv4Address& dest) const {
...
// MOH Added
IPv4Route *IPv4RoutingTable::findBestMatchingRouteEcmp(IPv4Datagram *datagram, const IPv4Address& dest) {
...
//    std::cout << " \n\n srcAddr " <<  srcAddr.str()  << std::endl;
//    std::cout << "   destAddr " <<  destAddr.str()  << std::endl;
...
//    raptorq::RQSegment *tcpseg = check_and_cast<raptorq::RQSegment *>(datagram->getEncapsulatedPacket());

//std::cout << " ip4routing \n\n";
           unsigned int srcPort=tcpseg->getSourcePort();
           unsigned int destPort=tcpseg->getDestinationPort();

//        unsigned int srcPort=1025;
//        unsigned int destPort=80;





//     if (dynamic_cast<tcp::TCPSegment *>(datagram)){
//          unsigned short    srcPort= check_and_cast<tcp::TCPSegment *>(datagram->getEncapsulatedPacket())->getSrcPort();
//         unsigned short  destPort= check_and_cast<tcp::TCPSegment *>(datagram->getEncapsulatedPacket())->getDestPort();
//     }


//    std::cout << "   tcpseg getMulticastGroupIndex " <<  tcpseg->getMulticastGroupIndex() << std::endl;

//     std::cout << " getSimulation()->getModule->getFullName() " << getSimulation()->getContextModule()->getFullPath() << std::endl;
...
//      hashValue = floor(hashValue/1000000);
...
// MOH: alternative way to hash
//    std::hash<std::string> ipSrc_hash;
//    std::hash<std::string> ipDest_hash;
//    std::hash<int> portSrc_hash;
//    std::hash<int> portDest_hash;
//    size_t hashValue= (ipSrc_hash(srcAddr.str())*59 )^ ipDest_hash(destAddr.str())^ (portSrc_hash(srcPort<<16))^ (portDest_hash(destPort))^transportProtocol;

    IPv4Route *bestRoute = nullptr;
    //std::vector<IPv4Route *> bestRouteEcmp;

    range = routingCache.equal_range(dest);
    // int numPossibleEcmpRoutesCaches= routingCache.count(dest);                      // slower
    int numPossibleEcmpRoutesCaches = std::distance(range.first, range.second);// faster

   // std::cout << " \n \n \n \n " << std::endl;

   // std::cout << " newwwwwww " << getFullPath() << std::endl;
   // std::cout << " destAddr.str() =  " << destAddr.str() << std::endl;
   // std::cout << " srcAddr.str() =  " << srcAddr.str() << std::endl;
    if (numPossibleEcmpRoutesCaches == 0) {
        for (auto e : routes) {
            EV_DETAIL << "hiiii find best match" << e->getDestination() << "\n";
      //      std::cout << "hiiii find best match" << e->getDestination() << "\n";

            if (e->isValid()) {
                if (IPv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask())) { // match
                    bestRoute = const_cast<IPv4Route *>(e);
                    // bestRouteEcmp.push_back(bestRoute);

                    //  routingCache[dest] = bestRoute;  // MOH: we use multimap  (use insert,see below) now not map
                    routingCache.insert (std::pair<IPv4Address, IPv4Route *>(dest,bestRoute));
         //           std::cout << "hiiii cached " << e->getDestination() << "\n";

//                      std::cout << "bestroute=  " << bestRoute->getGateway() << std::endl;
//                       std::cout << "bestroute=  " << bestRoute->getInterfaceName() << std::endl;
                    // std::cout << "bestroute=  " << bestRoute->getNetmask() << std::endl;
        ...
        //       numPossibleEcmpRoutesCaches= bestRouteEcmp.size();
    }

    if(numPossibleEcmpRoutesCaches==1) {
         auto it = range.first;
//         std::cout << "bestroute getGateway=  " << it->second->getGateway() << " , interface= " << it->second->getInterfaceName() << std::endl;
//         std::cout << "\n\n numPossibleEcmpRoutesCaches==1 "  << std::endl;
...
//        std::cout << "\n\n hashValueIII " << hashValueIII << std::endl;

//        for (auto it = range.first; it != range.second; ++it)
//        {
//            std::cout << " range= range= range= range= range= range= range= range= range= range=  "  << std::endl;
//            std::cout << "dest address =  " << it->first.str() <<"\n";
//            std::cout << "bestroute getGateway=  " << it->second->getGateway() << " , interface= " << it->second->getInterfaceName() << std::endl;
//        }
//        EV_INFO << "\n\\n "  << std::endl;
//        EV_INFO << "hashValue=  " << hashValue << std::endl;
//        EV_INFO << "numPossibleEcmpRoutesCaches=  " << numPossibleEcmpRoutesCaches << std::endl;
//        EV_INFO << "hashValue%numPossibleEcmpRoutesCaches=  " << hashValue%numPossibleEcmpRoutesCaches << std::endl;
//        std::cout  << "numPossibleEcmpRoutesCaches=  " << numPossibleEcmpRoutesCaches << std::endl;
//        std::cout  << "hashValue%numPossibleEcmpRoutesCaches=  " << hashValue%numPossibleEcmpRoutesCaches << std::endl;
//        std::cout << "hashValueIII " << hashValueIII << std::endl;

        // srand(time(NULL)); // wrong bugggg
...
//        std::cout  << "hashValue%numPossibleEcmpRoutesCaches=  " << selected << std::endl;
...
//            std::cout << " i = " << i<< " \n "   ;
            ...
            if ( i == selected) {
//                std::cout << " i = " << i<< " \n "   ;
       //         std::cout << "bestroute getGateway=  " << it->second->getGateway() << std::endl;
//                std::cout << "bestroute=  " << it->second->getInterfaceName() << std::endl;
                return it->second;
            }
            i++;
        }
...
InterfaceEntry *IPv4RoutingTable::getInterfaceForDestAddr(
        const IPv4Address& dest) const {
...
IPv4Address IPv4RoutingTable::getGatewayForDestAddr(
        const IPv4Address& dest) const {
...
const IPv4MulticastRoute *IPv4RoutingTable::findBestMatchingMulticastRoute(
        const IPv4Address& origin, const IPv4Address& group) const {
...
IPv4Route *IPv4RoutingTable::getDefaultRoute() const {
    ...
    for (RouteVector::const_reverse_iterator i = routes.rbegin();
            i != routes.rend() && (*i)->getNetmask().isUnspecified(); ++i) {
        if ((*i)->isValid())
            return *i;
    }
    return nullptr;
...
bool IPv4RoutingTable::routeLessThan(const IPv4Route *a,
        const IPv4Route *b) const {
...
// MOD added
//void IPv4RoutingTable::getRouterId(IPv4Address& a) {
//     a =  routerId  ;
//}
...
void IPv4RoutingTable::internalAddRoute(IPv4Route *entry) {
    if (!entry->getNetmask().isValidNetmask())
        throw cRuntimeError("addRoute(): wrong netmask %s in route",
                entry->getNetmask().str().c_str());

    if (entry->getNetmask().getInt() != 0
    ...
    auto pos = upper_bound(routes.begin(), routes.end(), entry,
            RouteLessThan(*this));
...
bool IPv4RoutingTable::multicastRouteLessThan(const IPv4MulticastRoute *a,
...
void IPv4RoutingTable::internalAddMulticastRoute(IPv4MulticastRoute *entry) {
    if (!entry->getOriginNetmask().isValidNetmask())
        throw cRuntimeError(
                "addMulticastRoute(): wrong netmask %s in multicast route",
                entry->getOriginNetmask().str().c_str());

    if ((entry->getOrigin().getInt() & ~entry->getOriginNetmask().getInt())
            != 0)
        throw cRuntimeError(
                "addMulticastRoute(): suspicious route: origin IP address %s has bits set outside netmask %s",
                entry->getOrigin().str().c_str(),
                entry->getOriginNetmask().str().c_str());

    if (!entry->getMulticastGroup().isUnspecified()
            && !entry->getMulticastGroup().isMulticast())
        throw cRuntimeError(
                "addMulticastRoute(): group address (%s) is not a multicast address",
                entry->getMulticastGroup().str().c_str());

    // check that the interface exists
    if (entry->getInInterface()
            && !entry->getInInterface()->getInterface()->isMulticast())
        throw cRuntimeError(
                "addMulticastRoute(): input interface must be multicast capable");

    for (unsigned int i = 0; i < entry->getNumOutInterfaces(); i++) {
        IPv4MulticastRoute::OutInterface *outInterface = entry->getOutInterface(
                i);
        if (!outInterface)
            throw cRuntimeError(
        ...
        else if (entry->getInInterface()
                && outInterface->getInterface()
                        == entry->getInInterface()->getInterface())
            throw cRuntimeError(
    ...
    // we keep entries sorted by netmask desc, metric asc in routeList, so that we can
    // stop at the first match when doing the longest netmask matching
    auto pos = upper_bound(multicastRoutes.begin(), multicastRoutes.end(),
            entry, multicastRouteLessThan);
...
IPv4MulticastRoute *IPv4RoutingTable::internalRemoveMulticastRoute(
        IPv4MulticastRoute *entry) {
...
IPv4MulticastRoute *IPv4RoutingTable::removeMulticastRoute(
        IPv4MulticastRoute *entry) {
...
void IPv4RoutingTable::routeChanged(IPv4Route *entry, int fieldCode) {
    if (fieldCode == IPv4Route::F_DESTINATION
            || fieldCode == IPv4Route::F_PREFIX_LENGTH
            || fieldCode == IPv4Route::F_METRIC) { // our data structures depend on these fields
        entry = internalRemoveRoute(entry);
...
void IPv4RoutingTable::multicastRouteChanged(IPv4MulticastRoute *entry,
        int fieldCode) {
    if (fieldCode == IPv4MulticastRoute::F_ORIGIN
            || fieldCode == IPv4MulticastRoute::F_ORIGINMASK
            || fieldCode == IPv4MulticastRoute::F_MULTICASTGROUP
            || fieldCode == IPv4MulticastRoute::F_METRIC) // our data structures depend on these fields
                    {
        entry = internalRemoveMulticastRoute(entry);
        ASSERT(entry != nullptr); // failure means inconsistency: route was not found in this routing table
        internalAddMulticastRoute(entry);
...
void IPv4RoutingTable::updateNetmaskRoutes() {
    ...
    for (int i = 0; i < ift->getNumInterfaces(); i++) {
        InterfaceEntry *ie = ift->getInterface(i);
        if (interfaceNameMatcher.matches(ie->getFullName()) && ie->ipv4Data()
                && ie->ipv4Data()->getNetmask()
            ...
            route->setDestination(
                    ie->ipv4Data()->getIPAddress().doAnd(
                            ie->ipv4Data()->getNetmask()));
            ...
            auto pos = upper_bound(routes.begin(), routes.end(), route,
                    RouteLessThan(*this));
...
bool IPv4RoutingTable::handleOperationStage(LifecycleOperation *operation,
