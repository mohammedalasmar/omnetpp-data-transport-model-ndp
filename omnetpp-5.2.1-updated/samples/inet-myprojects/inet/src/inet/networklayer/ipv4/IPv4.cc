...
//    std::cout << "Received datagram aaaaaa`" << datagram->getName() << "' with dest=" << datagram->getDestAddress() << "\n";
//    std::cout<< "Received datagram aaaaaa`" << datagram->getName() << "' with src=" << datagram->getSrcAddress() << "\n";

    EV_DETAIL << "Received datagram aaaaaa`" << datagram->getName() << "' with dest=" << datagram->getDestAddress() << "\n";
    EV_DETAIL << "Received datagram aaaaaa`" << datagram->getName() << "' with src=" << datagram->getSrcAddress() << "\n";

    std::string thisModule = getParentModule()->getParentModule()->getFullPath();
//          std::cout << "ipv4444444 thisModule >>>>> " << thisModule << " \n";



//          L3Address ttt;
//          L3AddressResolver().tryResolve(thisModule.c_str()  , ttt );
//          std::cout <<" thisModule ipv444444444  "<<ttt.toIPv4() << " \n";
...
//    std::cout << "handlePacketFromHL \n";
    ...
    //MOH
//      std::string initEdgeDest = "FatTree.Pod[" + std::to_string(1) + "].racks[" + std::to_string(1) + "].edgeSwitch";
//      L3Address destination;
//      L3AddressResolver().tryResolve(initEdgeDest.c_str(), destination);
//      std::cout << "destination  " << destination  << "\n";
//      controlInfo->setDestAddr(destination.toIPv4());
//    IPv4Datagram *datagram = encapsulate(packet, controlInfo);



    // MOH
//    raptorq::RQSegment *rqSeg = check_and_cast<raptorq::RQSegment *>(datagram->getEncapsulatedPacket());
//   if(rqSeg) std::cout << " dasdasdasdadasdttttttttttttttttt " <<  rqSeg->getMulticastGroupIndex()  << " \n";
...
//    raptorq::RQSegment *rqSeg = nullptr;
//
//    rqSeg = dynamic_cast<raptorq::RQSegment *>(datagram->getEncapsulatedPacket());
    ...
    if (destIE) {
       ...
       // const IPv4Route *re = rt->findBestMatchingRoute(destAddr);                  // MOH: commented
        const IPv4Route *re = rt->findBestMatchingRouteEcmp( datagram,destAddr);     // MOH: added
