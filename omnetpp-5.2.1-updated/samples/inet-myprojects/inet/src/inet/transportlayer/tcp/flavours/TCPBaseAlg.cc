...
TCPBaseAlgStateVariables::TCPBaseAlgStateVariables()
{
    ...
    numFastRetransmissions=0; // added moh
...
TCPBaseAlg::TCPBaseAlg() :
        TCPAlgorithm(), state((TCPBaseAlgStateVariables *&) TCPAlgorithm::state)
{
...
//    conn->getTcpMain()->appGateIndexTimeOutMap[conn->appGateIndex] = state->numRtos; // moh added
//    conn->getTcpMain()->numFastRetransmissionsMap[conn->appGateIndex] = state->numFastRetransmissions; // moh added
   conn->getTcpMain()->appGateIndexTimeOutMap[conn->connId] = state->numRtos; // moh added
  conn->getTcpMain()->numFastRetransmissionsMap[conn->connId] = state->numFastRetransmissions; // moh added
...
//    std::cout << "Performing retransmission #" << state->rexmit_count << "; increasing RTO from " << state->rexmit_timeout << "s ";
...
//    std::cout << " to " << state->rexmit_timeout << "s, and cancelling RTT measurement\n";
...
//    std::cout << " current node: " << cSimulation::getActiveSimulation()->getContextSimpleModule()->getFullPath() << std::endl;
//    std::cout << " conn->appGateIndex " << conn->appGateIndex << " \n";
//    std::cout << "state->numRtos " << state->numRtos << "\n";
//    std::cout << "conn->localPort: " << conn->localPort << "   \n";

    if (numRtosVector)  numRtosVector->record(state->numRtos);

 // MOH fill in the map
//    conn->getTcpMain()->appGateIndexTimeOutMap[conn->appGateIndex] = state->numRtos  ;
    conn->getTcpMain()->appGateIndexTimeOutMap[conn->connId] = state->numRtos  ;
//    std::cout << "mappppp:                 \n";
//    for (std::map<int, int>::iterator iter = conn->getTcpMain()->appGateIndexTimeOutMap.begin(); iter != conn->getTcpMain()->appGateIndexTimeOutMap.end(); ++iter) {
//        std::cout << "mmmm  appGateIndex " << iter->first << "===> ";
//        std::cout << "mmmmm  numRtos  " << iter->second << " \n ";
//    }
...
void TCPBaseAlg::receiveSeqChanged()
{
    ...
    if (state->full_sized_segment_counter == 0 && !state->ack_now && state->last_ack_sent == state->rcv_nxt && !delayedAckTimer->isScheduled()) {    // ackSent?
        ...
        if (!state->delayed_acks_enabled) {    // delayed ACK disabled
            ...
            conn->sendAck();
            EV_INFO << " \n\n\n\n\n\n\n\n\n receivedDataAck receivedDataAck receivedDataAck  \n\n\n\n\n\n";
