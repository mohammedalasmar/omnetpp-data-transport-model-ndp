...
//    std::cout << "aaaaaaaaaaaaaaaa  recalculateSlowStartThreshold " << state->ssthresh << std::endl;
    if (ssthreshVector)
...
//    std::cout << "state->ssthreshstate->ssthreshstate->ssthreshstate->ssthreshstate->ssthreshstate->ssthresh  " << state->ssthresh << std::endl;
//    std::cout << "state->cwnd  " <<  this  << " " <<  state->snd_cwnd << std::endl;
...
//            std::cout<< "cwnd <= ssthresh: Slow Start: increasing cwnd by one SMSS bytes to " << std::endl;
...
void TCPReno::receivedDuplicateAck()
{
    ...
    if (state->dupacks == DUPTHRESH) {    // DUPTHRESH = 3
        ...
        // moh
        state->numFastRetransmissions++;

//        conn->getTcpMain()->numFastRetransmissionsMap[conn->appGateIndex] = state->numFastRetransmissions  ;
        conn->getTcpMain()->numFastRetransmissionsMap[conn->connId] = state->numFastRetransmissions  ;
//        std::cout << "TCPReno::receivedDuplicateAck()    numFastRetransmissions=  " << state->numFastRetransmissions<< std::endl;
