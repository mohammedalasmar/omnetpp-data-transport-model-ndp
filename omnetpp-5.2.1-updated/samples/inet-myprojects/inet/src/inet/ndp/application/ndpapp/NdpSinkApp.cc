...
//        if(isLongFlow)   std::cout << "bytesRcvd  " << bytesRcvd  << std::endl;

        if(isLongFlow)  tEndAdded = simTime();
        cPacket *pk = PK(msg);
        long packetLength = pk->getByteLength();
        if(isLongFlow == false)  bytesRcvd += packetLength; // we don't want to measure goodput for long flows nowwww
//        ++bytesRcvd;
...
//     MY_COUT << " NdpSinkApp::finish() fffffffffffffffffffffffffffffffffffffffffffffffffffff \n\n\n";
    ...
    double FCT = SIMTIME_DBL(tEndAdded - tStartAdded);
     ...
     if (isLongFlow == true) {
         std::cout << " <<<<< longflow recevid bytes = " << bytesRcvd << " \n";
         std::cout << " <<< throughput throughput longflow = " << throughput <<"\n\n\n";
     }
