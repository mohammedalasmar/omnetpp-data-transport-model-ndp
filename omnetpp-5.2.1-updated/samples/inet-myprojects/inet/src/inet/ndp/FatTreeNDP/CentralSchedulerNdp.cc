...
//            scheduleNewManyToOneSingleBlock(newDest , senders ,  i);
              scheduleNewManyToOneMultiBlock(newDest , senders ,  i);
        }
    }
    ////////
...
void CentralSchedulerNdp::getNewOutcast(std::string& itsSrc, std::string& newDest, unsigned int newSrc, unsigned int destNode)
{
    std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  \n";
...
void CentralSchedulerNdp::scheduleManyToOneLongFlowsBackgroundTraffice()
{
    std::cout << "\n\n ******************** scheduleLongFlows .. ********************  \n";
    std::string dest;
    std::string source;
...
//        cout << "  nodePodRackLoc:  " << iter->second << " == " << source << " ==> " << iter->first << " == " << dest << "\n";
    ...
    if (strcmp(trafficMatrixType, "randTM") == 0)  recordScalar("randTM", 0);
...
//             getSimulation()->callFinish();
        ...
        if (oneToMany == false && manyToOne == true && oneToOne == false && numCompletedShortFlows ==  numRunningMultiSourcingGroups) {
            scheduleAt(simTime(), stopSimulation);
        }
    }
}
...
//          Receiver
//   --------        ------------
//   |      |        |   conn1  |   <------------------    Sender 1 (NdpBasicClientApp)
//   | SINK |  <---- |   conn2  |   <------------------    Sender 2 (NdpBasicClientApp)
//   |      |        |   conn3  |   <------------------    Sender 3 (NdpBasicClientApp)
//   --------        ------------
...
// to use this way take  WAY-I block outside the for loop
  ...
  ////////////////////////////  WAY-I  /////////////////////////////////////
    ...
    for (auto iter = senders.begin(); iter != senders.end(); ++iter) { // ndp many-to-one multi-blocks
//  auto iter = senders.begin(); // ndp many-to-one single-block fetch
        // >>>>>>>>>>>>>>>>>>>> >>>>>>>>>>> >>>>>>>>>>> >>>>>>>>>>> >>>>>>>>>>>
        ...
        newSrcAppModule->par("startTime").setDoubleValue(simTime().dbl() + sumArrivalTimes);
        newSrcAppModule->par("isLongFlow").setBoolValue(false);
...
void CentralSchedulerNdp::scheduleNewManyToOneSingleBlock(std::string dest, std::vector<std::string> senders, unsigned int multiSrcGroupId) // three replicas
{
    std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@ scheduleNewMultiSourcingSession fetch three replicas  multi sourcing .. @@@@@@@@@@@@@@@@@@@@@@@@@  \n";
    std::cout << " dest nodes: " << dest << "\n";
...
void CentralSchedulerNdp::scheduleNewManyToOneMultiBlock(std::string dest, std::vector<std::string> senders, unsigned int multiSrcGroupId) // three replicas
{
    std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@ scheduleNewMultiSourcingSession fetch three replicas  multi sourcing .. @@@@@@@@@@@@@@@@@@@@@@@@@  \n";
    std::cout << " dest nodes: " << dest << "\n";
