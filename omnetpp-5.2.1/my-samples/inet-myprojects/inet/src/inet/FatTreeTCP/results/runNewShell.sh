...
opp_run_release -u Cmdenv -m -n ../../..:../../../../examples   -l ../../../INET -f ../centralSchTCP.ini -c General -r "\$FatTreeSize==$(printf '%d' $1)  &&  \$numShortFlows==$(printf '%d' $2)"
...
scavetool export -T s -f " module(**.tcpApp[*])  AND ("multicastGroupIdSignalTcp:last") "   -F CSV-S -o multicastgroups.csv *.sca
scavetool export -T s -f " module(**.tcpApp[*])  AND ("multisourceGroupIdSignalTcp:last") "   -F CSV-S -o multisourcegroups.csv *.sca
...
echo "config."
scavetool export -T s -f "name(simTimeTotal=)  OR name(numShortFlows=)  OR name(flowSize=) OR name(percentLongFlowNodes=) OR name(arrivalRate=) OR name(randTM)  OR name(permTM)  OR name(wallClockTime=)  OR name(numReplicas=) OR name(numMulticastSessions=)  OR   name(numMultisourceSessions=) "   -F CSV-S -o config.csv *.sca
