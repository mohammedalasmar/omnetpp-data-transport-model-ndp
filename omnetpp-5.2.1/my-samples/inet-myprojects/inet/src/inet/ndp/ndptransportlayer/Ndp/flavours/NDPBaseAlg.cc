...
//
// RFC 1122, page 95:
// "A NDP SHOULD implement a delayed ACK, but an ACK should not
...
NDPBaseAlgStateVariables::NDPBaseAlgStateVariables() {
    rexmit_count = 0;
...
NDPBaseAlg::NDPBaseAlg() :
        NDPAlgorithm(), state((NDPBaseAlgStateVariables *&) NDPAlgorithm::state) {
...
NDPBaseAlg::~NDPBaseAlg() {
    // Note: don't delete "state" here, it'll be deleted from NDPConnection
...
void NDPBaseAlg::established(bool active) {
    ...
    // (MSS) of 1460 bytes (assuming no IP or NDP options).  Therefore,
    ...
    //     then win <= 2 * MSS;
    //
    // This increased initial window is optional: a NDP MAY start with a
    // larger initial window.  However, we expect that most general-purpose
    // NDP implementations would choose to use the larger initial congestion
...
void NDPBaseAlg::connectionClosed() {
...
void NDPBaseAlg::processTimer(cMessage *timer, NDPEventCode& event) {
...
void NDPBaseAlg::processRexmitTimer(NDPEventCode& event) {
    ...
    // in RFC 793 above, we'll leave it to subclasses (e.g. NDPTahoe, NDPReno).
    //
...
//        conn->signalConnectionTimeout();
        event = NDP_E_ABORT;  // TBD maybe rather introduce a NDP_E_TIMEDOUT event
        return;
    }
...
//    state->time_last_data_sent = simTime();
    ...
    // subclasses (e.g. NDPTahoe, NDPReno).
    //
...
void NDPBaseAlg::processPersistTimer(NDPEventCode& event) {
    ...
    // as presented in [Stevens, W.R.: NDP/IP Illustrated, Volume 1, chapter 22.2]
    ...
    state->persist_timeout = state->persist_factor * 1.5; // 1.5 is a factor for typical LAN connection [Stevens, W.R.: NDP/IP Ill. Vol. 1, chapter 22.2]
...
void NDPBaseAlg::processDelayedAckTimer(NDPEventCode& event) {
//    state->ack_now = true;
//    conn->sendAck();
}

void NDPBaseAlg::processKeepAliveTimer(NDPEventCode& event) {
    // TBD
    // RFC 1122, page 102:
    // "A "keep-alive" mechanism periodically probes the other
    // end of a connection when the connection is otherwise
    // idle, even when there is no data to be sent.  The NDP
...
void NDPBaseAlg::startRexmitTimer() {
...
void NDPBaseAlg::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked) {
...
//    //
    ...
    // 500ms ticks is available from old NDPmodule.cc:calcRetransTimer().
    //
...
void NDPBaseAlg::rttMeasurementCompleteUsingTS(uint32 echoedTS) {
...
bool NDPBaseAlg::sendData(bool sendCommandInvoked) {
    //
    // Nagle's algorithm: when a NDP connection has outstanding data that has not
    ...
    // by NDP and sent in a single segment.)
    //
...
//
//    // RFC 2581, pages 7 and 8: "When NDP has not received a segment for
...
//    // (...)
//    // Therefore, a NDP SHOULD set cwnd to no more than RW before beginning
//    // transmission if the NDP has not sent data in an interval exceeding
...
void NDPBaseAlg::sendCommandInvoked(cMessage *msg) {

    // MOH Added
    GenericAppMsgNdp *appmsgg = dynamic_cast<GenericAppMsgNdp *>(msg);
...
//    numSymbolsToSend=appmsgg->getNumSymbolsToSend();
 //    std::cout << "  NDPBaseAlg::sendCommandInvoked,,  isReader " << isReader << " , isWriter "  << isWriter << std::endl ;
...
void NDPBaseAlg::receivedOutOfOrderSegment() {
//    state->ack_now = true;
    EV_INFO << "Out-of-order segment, sending immediate ACK\n";
//    conn->sendAck();
}

void NDPBaseAlg::receiveSeqChanged() {
...
//            // NDPEV << "ACK has already been sent (possibly piggybacked on data)\n";
//    } else {
//        // RFC 2581, page 6:
//        // "3.2 Fast Retransmit/Fast Recovery
//        // (...)
//        // In addition, a NDP receiver SHOULD send an immediate ACK
//        // when the incoming segment fills in all or part of a gap in the
//        // sequence space."
//        if (state->lossRecovery)
//            state->ack_now = true; // although not mentioned in [Stevens, W.R.: NDP/IP Illustrated, Volume 2, page 861] seems like we have to set ack_now
//
...
void NDPBaseAlg::receivedDataAck(uint32 firstSeqAcked) {
//    if (!state->ts_enabled) {
...
void NDPBaseAlg::receivedDuplicateAck() {
...
void NDPBaseAlg::receivedAckForDataNotYetSent(uint32 seq) {
    // Note: In this case no immediate ACK will be send because not mentioned
    // in [Stevens, W.R.: NDP/IP Illustrated, Volume 2, page 861].
    // To force immediate ACK use:
    // state->ack_now = true;
    // NDPEV << "ACK acks something not yet sent, sending immediate ACK\n";
    EV_INFO << "ACK acks something not yet sent, sending ACK\n";
//    conn->sendAck();
}

void NDPBaseAlg::ackSent() {
...
void NDPBaseAlg::dataSent(uint32 fromseq) {
...
void NDPBaseAlg::segmentRetransmitted(uint32 fromseq, uint32 toseq) {
}

void NDPBaseAlg::restartRexmitTimer() {
    if (rexmitTimer->isScheduled())
        cancelEvent(rexmitTimer);
...
// MOH Added
void NDPBaseAlg::cancelRexmitTimer() {
    if (rexmitTimer->isScheduled())
        cancelEvent(rexmitTimer);
}
